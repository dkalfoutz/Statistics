...
The Situation
Point estimates, such as the sample proportion ( ??p ), the sample mean ( x ), and the sample
variance (
2 s ) depend on the particular sample selected. For example:
(1) We might know that
??p , the proportion of a sample of 88 students who use the city bus daily
...
(2) We might know that x , the average number of credit cards of 32 randomly selected
American college students is 2.2. But, we want to know ì, the average number of credit cards
of all American college students.
The Problem
(1) When we use the sample mean x to estimate the population mean ì, can we be confident
that x is close to ì? And, when we use the sample proportion
??p to estimate the population
...
deriving a particular confidence interval, namely that of a population mean ì. We'll jump right
...
such that the area to the right of it under the standard normal curve is á/2. That is:
P(Z???zá/2)=á/2
Likewise:
???zá/2
is the Z-value (obtained from a standard normal table) such that the area to the left of it under the
standard normal curve is á/2. That is:
P(Z??????zá/2)=á/2
...
mean.
Theorem. Assume:
(1) X1, X2, ..., Xn is a random sample from a normal population with mean ì and variance ó2.
So that:
2
( , ) and (0,1)
/
...
???
???
(2) The population variance ó2 is known.
Then, a (1 ??? á)100% confidence interval for the mean ì is:
X z /2 ( )
...
reverse by calling it a (1-á)100% confidence level. Where for any á in (0,1) we can find a z* with
P(-z* < z < z*) = 1-á
Often such a z* is called z1-á/2 from how it is found. For R this can be found with the qnorm
function
...
fumes in downtown Cairo had an average blood lead level concentration of 29.2 ìg/dl.
Assume X, the blood lead level of a randomly selected policeman, is normally distributed with a
standard deviation of ó = 7.5 ìg/dl. Historically, it is known that the average blood lead level
concentration of humans with no exposure to automobile exhaust is 18.2 ìg/dl. Is there
...
population mean. For a 95% confidence interval, 1???á = 0.95, so that á = 0.05 and á/2 = 0.025.
Therefore, as the following diagram illustrates the situation, z0.025 = 1.96:
Now, substituting in what we know ( x = 29.2, n = 126, ó = 7.5, and z0.025 = 1.96) into the
...
exposed to constant auto exhaust is between 27.9 ìg/dl and 30.5 ìg/dl. Note that the interval
...
(1) The population standard deviation ó is known, and
...
deviation ó, but not the population mean ì? Let's entertain, then, the realistic situation in which
not only the population mean ì is unknown, but also the population standard deviation ó is
unknown.
What if ó is unknown?
Yes, the reasonable thing to do is to estimate the population standard deviation ó with the sample
standard deviation:
2
1
1
...
Theorem. If X1, X2, ..., Xn are normally distributed with mean ì and variance ó2, then:
/
X
T
S n
...
interval for a population mean in the realistic situation that ó is unknown.
...
Theorem. If X1, X2, ..., Xn are normally distributed random variables with mean ì and
variance ó2, then a (1???á)100% confidence interval for the population mean ì is:
X t /2 (S n) ??? ???
...
mean ì when the data are normally distributed. That is, the t-interval for ì (and Z-interval, for
...
The implication, therefore, is that the t-interval for ì:
/2, 1( ) a n
s
x t
n
??? ???
and the Z-interval for ì:
/2( ) a
s
x t
n
???
(with the sample standard deviation s replacing the unknown population standard deviation ó!)
...
```{r}
#  ... as ipothesoume oti to sigma einai gnwsto..
#      kai ta  X_i einai katanonika katanenhmena...
set.seed(9.1)
mu = 10
sd = 5
...
#  ... kai apo ena standard normal pinaka ,
#      vlepoume oti afthn h metrisi xondrika
#      mono 2.8% tou plithismou exei sample mean
#      wste toso na parekklinei  apo to  mean tou plithismou (10)
#      
```
...
case, the margin of error equals:
p(1 p)
z
n
???
